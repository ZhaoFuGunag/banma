<template>
    ...
    <van-tabs v-model:active="active" @click="change">


        <van-tab :title="item" v-for="item in arr">

            <van-list v-model:loading="loading" @load="onLoad">
                <div v-for="item in list1" class="wrap" :key="item.id" @click="tiao(item.url)">

                    <img v-lazy="item.head_pic" alt="">

                    <p>{{ item.title }}</p>
                    <p class="title1">{{ item.subtitle }}</p>
...
import { found } from '../tools/ajax'
const loading = ref(false)
const active1 = ref(3);
const page = ref(0)
const list1 = reactive([])
const arr = reactive(['全部', '涨知识', "看世界", "达人说"])
function onLoad() {
    //发请求
    found({
        sort_id: 0,
...
function change(name) {
    if (name == 0) {
        ...
        found({
            ...
            list1.length = ''
            list1.push(...res.data.list)
            // list.push(...res.data.list.slice(0, 3))
            // 数据请求完成, 需要结束加载状态, 之后才能触发触底的load事件
            loading.value = false
        })
