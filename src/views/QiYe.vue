<template>
    <div class="big">
        <img :src="arr.banner" alt="" class="xiaop">
        <Fu :arr="arr"></Fu>
        <div class="shire">
            <h2>案例分享</h2>
        </div>
        <van-tabs v-model:active="active" @click-tab="onClickTab">
            <van-tab v-for="(item, index) in arr1" :title="item.name" :name="index">
                <ul v-for="item in item.product_ids">
                    <router-link :to="'/two?id=' + item">

                        <li v-for="item in chu" class="nihao">
        ...
        <div>
            <img src="../assets/QQ图片20220720213938.png" alt="">
        </div>
    </div>
...
<script>
import { qiye, dingzhishi } from '../tools/ajax';
import Fu from "../components/Fu.vue"
import New from '@/components/New.vue';
export default {
    name: 'WorkspaceJsonQiYe',
    components: {
        Fu,
        New,
        // DiZi
    ...
    methods: {
        get() {
            qiye().then(res => {
                this.arr = res.data
                console.log(this.arr, '22223');
                this.arr1 = res.data.list
                // console.log(this.arr1, "11111");
                dingzhishi({
                    ids: this.arr1[this.active].product_ids
                }).then(res => {
                    // console.log(res);
                    this.chu = res.data.list
                })
            })
        },
        onClickTab() {
